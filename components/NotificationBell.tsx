...
// Renders the bell icon SHELL immediately (always, no conditional).
// Inside a logged-in branch already gated by NavAuthSlot's data-auth-state="in" CSS rule,
// so this component is only painted for logged-in users.
// Fetches notifications in background; only the count badge updates without layout shift.
export default function NotificationBell() {
  ...
  // Bell SHELL renders unconditionally — no "if (!auth) return null"
  // The CSS data-auth-state="in" gate in Nav.tsx ensures this only paints for logged-in users
  return (
        ...
        <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '0.5rem', backgroundColor: '#fff', border: '1px solid var(--color-border)', width: '300px', zIndex: 100, boxShadow: '0 4px 16px rgba(0,0,0,0.12)' }}>
          ...
          {notifications.length === 0 ? (
            ...
            <div style={{ maxHeight: '320px', overflowY: 'auto' }}>
              {notifications.map(n => (
                <a key={n.id} href={n.link || '#'} onClick={() => markRead(n.id)}
