    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.875rem 0', borderTop: '1px solid var(--color-border)', borderBottom: '1px solid var(--color-border)', margin: '1.5rem 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
          <a href="#" onClick={e => { e.preventDefault(); const ua = navigator.userAgent; if (/iPhone|iPad|iPod/i.test(ua)) { window.location.href = `fb://share?u=${encoded}`; } else if (/Android/i.test(ua)) { window.location.href = `intent://www.facebook.com/dialog/share?app_id=2107832130079548&href=${encoded}#Intent;package=com.facebook.katana;scheme=https;end`; } else { window.open(`https://www.facebook.com/dialog/share?app_id=2107832130079548&display=popup&href=${encoded}&quote=${encodedTitle}`, '_blank'); } }} title="Share on Facebook" style={{ color: '#9a9085', display: 'flex', textDecoration: 'none' }}>
