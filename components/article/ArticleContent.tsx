    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.875rem 0', borderTop: '1px solid var(--color-border)', borderBottom: '1px solid var(--color-border)', margin: '1.5rem 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
          <a href={`https://www.facebook.com/dialog/share?app_id=2107832130079548&display=popup&href=${encoded}&quote=${encodedTitle}`} target="_blank" rel="noopener noreferrer" title="Share on Facebook" style={{ color: '#9a9085', display: 'flex', textDecoration: 'none' }} onClick={e => { if (typeof navigator !== 'undefined' && /iPhone|iPad|iPod|Android/i.test(navigator.userAgent)) { e.preventDefault(); if (navigator.share) { navigator.share({ title: article.title, url: url }); } else { window.location.href = 'fb://share?u=' + encodeURIComponent(url); } } }}>
