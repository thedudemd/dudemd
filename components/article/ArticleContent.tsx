...
export default function ArticleContent({ article, slug, category, relatedArticles = [], parentPillar }: { article: any, slug: string, category: string, relatedArticles?: any[], parentPillar?: any }) {
  ...
  function fireShareEvent() {
    ...
    if (auth?.uid && auth?.token) {
      try { fetch('/api/personalization/score', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: auth.uid, token: auth.token, event_type: 'share', article_slug: slug, category_slug: category }) }) } catch(e) {}
    }
  }
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.875rem 0', borderTop: '1px solid var(--color-border)', borderBottom: '1px solid var(--color-border)', margin: '1.5rem 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
          ...
          <span className="share-tip" data-tip="Post on X"><a href={`https://twitter.com/intent/tweet?url=${encoded}&text=${encodedTitle}`} target="_blank" rel="noopener noreferrer" title="Post on X" onClick={fireShareEvent} style={{ color: '#9a9085', display: 'flex', textDecoration: 'none' }}>
          ...
          <span className="share-tip" data-tip="Share on LinkedIn"><a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encoded}`} target="_blank" rel="noopener noreferrer" title="Share on LinkedIn" onClick={fireShareEvent} style={{ color: '#9a9085', display: 'flex', textDecoration: 'none' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
          </a></span>
          <span className="share-tip" data-tip="Share on Threads"><a href={`https://www.threads.net/intent/post?text=${encodedTitle}%20${encoded}`} target="_blank" rel="noopener noreferrer" title="Share on Threads" onClick={fireShareEvent} style={{ color: '#9a9085', display: 'flex', textDecoration: 'none' }}>
          ...
          <span className="share-tip" data-tip="Share to Circles"><a href={`/circles?from=${encodeURIComponent(url)}`} title="Share to Circles" style={{ color: "#9a9085", display: "flex", textDecoration: "none" }}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></a></span>
