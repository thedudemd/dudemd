...
export default function MagazineLayout({ article, children }: any) {
  return (
    <>
      <div style={{ position: 'relative', width: '100%', minHeight: '520px', overflow: 'hidden', display: 'flex', alignItems: 'flex-end' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '3rem 2rem', maxWidth: '800px', margin: '0 auto', width: '100%' }}>
          <p style={{ fontSize: '10px', fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#c9b28f', marginBottom: '0.75rem' }}>{article.categories?.name}</p>
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 700, color: '#f7f4ee', lineHeight: 1.15, marginBottom: '1rem' }}>{article.title}</h1>
