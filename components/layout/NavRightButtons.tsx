...
export default function NavRightButtons({ navItems }: { navItems: any[] }) {
  ...
  const [session, setSession] = useState<'loading'|null|{uid:string}>('loading')
  ...
  // Fixed-width container — never changes size, prevents logo/category shift
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: '130px', justifyContent: 'flex-end' }}>
        {/* Auth button — fixed size slot, invisible until resolved */}
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', paddingTop: '0.5rem' }}>
            {[
              ...
              <a key={label} href={href} target="_blank" rel="noopener noreferrer"
                style={{ color: 'rgba(247,244,238,0.5)', display: 'flex', alignItems: 'center' }} aria-label={label}>
