...
import GoldBarAuthSwap from './GoldBarAuthSwap'
    ...
    if (Array.isArray(cats)) {
      navItems = await Promise.all(cats.map(async (cat: any) => {
        const [subsRes, artsRes] = await Promise.all([
          ...
          fetch(
            `${SUPABASE_URL}/rest/v1/articles?select=slug,title,cover_image_url&category_id=eq.${cat.id}&published=eq.true&order=published_at.desc&limit=2`,
            { headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}` }, next: { revalidate: 300 } }
          )
        ])
      ...
      <header style={{ backgroundColor: 'var(--color-navy)', position: 'sticky', top: 0, zIndex: 20 }}>
        <div className="container-content">
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 0' }}>
            ...
            {/* Logo — server HTML, paints instantly */}
            ...
            {/* Category links — server HTML, paints instantly */}
            ...
            {/* Right buttons — client only, hydrates after load */}
            <NavRightButtons navItems={navItems} />
          </div>
        </div>

        {/* Gold bar — static server HTML, always visible instantly */}
        <div style={{ position: 'relative', backgroundColor: 'var(--color-gold)', padding: '0.4rem 0' }}>
          ...
          {/* Swaps in auth-aware content once session resolves — no layout shift */}
          <GoldBarAuthSwap />
        </div>
      </header>
    </>
