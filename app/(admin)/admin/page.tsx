...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const { data } = await supabase.from('articles').select('*, authors(name), categories(name)').order('created_at', { ascending: false })
      setArticles(data || [])
      setLoading(false)
    }
    init()
  ...
  if (loading) return <div style={{ minHeight: '100vh', backgroundColor: '#0e1a2b', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><p style={{ color: '#f7f4ee', fontSize: '14px' }}>Loading...</p></div>
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #ede8df' }}>
          ...
          {articles.length === 0 ? (
            ...
            <div key={a.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 120px', gap: '1rem', padding: '1rem 1.5rem', borderBottom: i < articles.length - 1 ? '1px solid #ede8df' : 'none', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: a.published ? '#2d7a3a' : '#9a9085', backgroundColor: a.published ? '#e8f5ea' : '#f0ede8', padding: '0.25rem 0.5rem', display: 'inline-block' }}>{a.published ? 'Published' : 'Draft'}</span>
