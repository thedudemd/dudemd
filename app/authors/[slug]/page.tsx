    ...
    <main style={{ backgroundColor: 'var(--color-cream)', minHeight: '100vh' }}>
      ...
      <div style={{ backgroundColor: 'var(--color-navy)', padding: '3rem 0' }}>
        <div className="container-content" style={{ maxWidth: '900px' }}>
          ...
          <div style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
            {author.avatar_url ? (
              <img src={author.avatar_url} alt={author.name} style={{ width: 120, height: 120, borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--color-gold)', flexShrink: 0 }} />
            ) : (
              <div style={{ width: 120, height: 120, borderRadius: '50%', backgroundColor: 'var(--color-gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: '3px solid var(--color-gold)' }}>
                <span style={{ fontSize: 48, fontWeight: 700, color: 'var(--color-navy)' }}>{author.name?.charAt(0)}</span>
              </div>
            )}
            <div style={{ flex: 1, minWidth: '200px' }}>
              <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--color-gold)', marginBottom: '0.5rem' }}>Author</p>
              <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 700, color: 'var(--color-cream)', marginBottom: '0.5rem' }}>{author.name}</h1>
      ...
      <div className="container-content" style={{ maxWidth: '900px', paddingTop: '3rem', paddingBottom: '4rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '2rem', borderBottom: '2px solid var(--color-navy)', paddingBottom: '0.75rem' }}>
          <h2 style={{ fontFamily: 'Georgia, serif', fontSize: '1.25rem', fontWeight: 700, color: 'var(--color-navy)' }}>Articles by {author.name.split(' ')[0]}</h2>
        ...
        {articles.length === 0 ? (
          ...
          <div className="author-grid">
            {articles.map((article: any) => (
              <Link key={article.slug} href={article.external_url || `/articles/${article.categories?.slug}/${article.slug}`} className="article-card" style={{ textDecoration: 'none', backgroundColor: '#fff', border: '1px solid var(--color-border)', display: 'block' }}>
                {article.cover_image_url && (
                  <div style={{ position: 'relative', overflow: 'hidden', height: '180px' }}>
                    <Image src={article.cover_image_url} alt={article.title} fill sizes="(min-width: 900px) 300px, 100vw" style={{ objectFit: 'cover', transition: 'transform 0.3s ease' }} />
                  </div>
                )}
                <div style={{ padding: '1.25rem' }}>
                  {article.categories?.name && (
                    <span style={{ fontSize: '10px', fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--color-gold)', display: 'block', marginBottom: '0.5rem' }}>{article.categories.name}</span>
                  )}
                  <h3 style={{ fontFamily: 'Georgia, serif', fontSize: '1rem', fontWeight: 700, color: 'var(--color-navy)', lineHeight: 1.4, marginBottom: '0.5rem' }}>{article.title}</h3>
                  {article.excerpt && <p style={{ fontSize: '13px', color: 'var(--color-slate)', lineHeight: 1.6, marginBottom: '0.75rem' }}>{article.excerpt.substring(0, 100)}{article.excerpt.length > 100 ? '...' : ''}</p>}
