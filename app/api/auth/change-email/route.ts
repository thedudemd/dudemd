...
export async function POST(req: NextRequest) {
  ...
  let confirmSubject = 'Confirm your new email address — DudeMD'
  let confirmHtml = `
      <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; background: #0e1a2b; padding: 2rem;">
        <div style="text-align: center; margin-bottom: 2rem;">
          <img src="https://bicljoujevywrkzjeaoy.supabase.co/storage/v1/object/public/media/article-images/1780803524618-pt9sjxvtbh.png" alt="DudeMD" style="height: 64px; width: auto;" />
        </div>
        <div style="background: #f7f4ee; padding: 2rem; border-radius: 4px;">
          ...
          <p style="color: #4A5563; line-height: 1.7; margin: 0 0 1.5rem;">You requested to change your DudeMD email address to <strong style="color: #0e1a2b;">{{new_email}}</strong>.</p>
          ...
          <div style="text-align: center; margin: 2rem 0;">
            <a href="{{confirm_link}}" style="display: inline-block; padding: 0.875rem 2.5rem; background: #0e1a2b; color: #f7f4ee; text-decoration: none; font-weight: 700; font-size: 0.85rem; letter-spacing: 0.1em; text-transform: uppercase; border: 2px solid #c9b28f;">Confirm Email Change</a>
          </div>
  ...
  const { data: customEmail } = await supabase.from('system_emails').select('subject, html').eq('key', 'email_change_confirm').single()
  if (customEmail && customEmail.html) {
    confirmSubject = customEmail.subject || confirmSubject
    confirmHtml = customEmail.html
  }
  confirmHtml = confirmHtml.split('{{confirm_link}}').join(confirmLink).split('{{new_email}}').join(newEmail)
