...
export async function POST(req: NextRequest) {
  ...
  let query = supabase.from('newsletter_subscribers').select('email').eq('status', 'active')

  if (segment && segment !== 'all' && segment !== '') {
    query = query.contains('subscribed_categories', [segment])
  }
  ...
  const emails: string[] = (subs || []).map(s => s.email)

  if (!emails.length) return NextResponse.json({ error: 'No subscribers', segment }, { status: 400 })
