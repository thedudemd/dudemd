...
export default function SignIn() {
  ...
  const inp = { width: '100%', padding: '0.75rem', border: '1px solid #ede8df', fontSize: '14px', outline: 'none', boxSizing: 'border-box' as const }
  const lbl = { display: 'block', fontSize: '12px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#4A5563', marginBottom: '0.5rem' }
    ...
    <main style={{ minHeight: '100vh', backgroundColor: '#f7f4ee', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: '400px' }}>
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #ede8df', padding: '2rem' }}>
          ...
          <button onClick={handleEmail} disabled={loading} style={{ width: '100%', padding: '0.875rem', backgroundColor: '#0e1a2b', color: '#f7f4ee', fontWeight: 700, fontSize: '13px', letterSpacing: '0.08em', textTransform: 'uppercase', border: 'none', cursor: 'pointer' }}>{loading ? 'Signing in...' : 'Sign In'}</button>
          <p style={{ fontSize: '13px', color: '#4A5563', textAlign: 'center', marginTop: '1.25rem' }}>Don't have an account? <Link href="/newsletter" style={{ color: '#c9b28f', fontWeight: 600 }}>Subscribe free</Link></p>
          <p style={{ fontSize: '12px', color: '#9a9085', textAlign: 'center', marginTop: '0.75rem' }}>By signing in you agree to our <Link href="/terms-of-use" style={{ color: '#c9b28f' }}>Terms</Link> and <Link href="/privacy-policy" style={{ color: '#c9b28f' }}>Privacy Policy</Link></p>
        </div>
      </div>
    </main>
  )
