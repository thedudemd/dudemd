...
export default function ArticlesPage() {
  ...
  useEffect(() => {
    supabase.from('articles').select('*, authors(name), categories(name)').order('created_at', { ascending: false }).then(({ data, error }) => {
      console.log('articles:', data, 'error:', error)
      setArticles(data || [])
    })
  }, [])
  ...
  async function handleDelete() {
    ...
    setDeleteId(null); setDeleteInput('')
  }
      ...
      {deleteId && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: '#fff', padding: '2rem', width: '100%', maxWidth: '420px', margin: '1rem' }}>
            ...
            <p style={{ fontSize: '13px', color: '#4A5563', lineHeight: 1.6, marginBottom: '1.25rem' }}>Type <strong>DELETE</strong> to confirm.</p>
