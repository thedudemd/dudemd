...
function ToolbarBtn({ active, onClick, title, children, disabled }: any) {
  return (
    <button type="button" onClick={onClick} disabled={disabled} title={title} style={{ padding: '5px 8px', fontSize: '12px', fontWeight: 600, backgroundColor: active ? '#0e1a2b' : 'transparent', color: active ? '#f7f4ee' : '#4A5563', border: 'none', cursor: disabled ? 'not-allowed' : 'pointer', borderRadius: 3, lineHeight: 1, opacity: disabled ? 0.4 : 1, whiteSpace: 'nowrap' as const }}>
      {children}
    </button>
  )
}

function EditV2Inner({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setForm({
        ...
        layout: article.layout || 'standard', tags: article.tags || [],
        show_hero: article.show_hero !== false,
        is_pillar_content: article.is_pillar_content || false,
        pillar_topic_id: article.pillar_topic_id || '',
  ...
  function setLink() {
    const url = window.prompt('URL:'); if (url === null) return
  ...
  async function handleSave(status: string) {
    ...
    const { error } = await supabase.from('articles').update({
      ...
      category_id: form.category_id || null, author_id: form.author_id || null,
      is_pillar_content: form.is_pillar_content, pillar_topic_id: form.pillar_topic_id || null,
  ...
  if (loading) return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><p style={{ color: '#9a9085' }}>Loading...</p></div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: '#f7f4ee', overflow: 'hidden' }}>
      ...
      <div style={{ backgroundColor: '#0e1a2b', padding: '0.6rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <span style={{ fontSize: '12px', color: 'rgba(247,244,238,0.7)', maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{form.title || 'Edit Article'}</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <button onClick={() => handleSave(form.status)} disabled={saving} style={{ padding: '0.5rem 1rem', fontSize: '12px', fontWeight: 700, color: 'rgba(247,244,238,0.7)', backgroundColor: 'transparent', border: '1px solid rgba(247,244,238,0.2)', cursor: 'pointer' }}>Save</button>
          {statusConf.next && (
            <button onClick={() => handleSave(statusConf.next!)} disabled={saving} style={{ padding: '0.5rem 1rem', fontSize: '12px', fontWeight: 700, color: '#0e1a2b', backgroundColor: '#c9b28f', border: 'none', cursor: 'pointer' }}>{statusConf.nextLabel}</button>
          )}
        </div>
      </div>
      ...
      <div style={{ backgroundColor: '#fff', borderBottom: '1px solid #e8e4de', padding: '4px 12px', display: 'flex', alignItems: 'center', flexWrap: 'wrap' as const, gap: '2px', flexShrink: 0, position: 'sticky' as const, top: 0, zIndex: 40, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        ...
        <ToolbarBtn active={editor?.isActive('paragraph')} onClick={() => editor?.chain().focus().setParagraph().run()} title="P">P</ToolbarBtn>
        <ToolbarDivider />
        ...
        <ToolbarBtn active={false} onClick={() => editor?.chain().focus().setHorizontalRule().run()} title="HR">— HR</ToolbarBtn>
        <ToolbarDivider />
        <ToolbarBtn active={editor?.isActive('link')} onClick={setLink} title="Link">🔗 Link</ToolbarBtn>
        <ToolbarBtn active={false} onClick={() => fileRef.current?.click()} disabled={uploading} title="Image">{uploading ? '...' : '🖼 Image'}</ToolbarBtn>
        ...
        <input type="color" title="Highlight" onChange={e => editor?.chain().focus().setHighlight({ color: e.target.value }).run()} style={{ width: 24, height: 24, border: '1px solid #e8e4de', padding: 1, cursor: 'pointer', backgroundColor: '#ffe066', borderRadius: 3 }} />
        <ToolbarBtn active={editor?.isActive({ textAlign: 'left' })} onClick={() => editor?.chain().focus().setTextAlign('left').run()} title="Left">⬅</ToolbarBtn>
        ...
        <ToolbarBtn active={editor?.isActive({ textAlign: 'right' })} onClick={() => editor?.chain().focus().setTextAlign('right').run()} title="Right">➡</ToolbarBtn>
        ...
        <ToolbarBtn active={showHtml} onClick={() => setShowHtml(!showHtml)} title="HTML">⟨/⟩</ToolbarBtn>
        <ToolbarBtn active={focusMode} onClick={() => setFocusMode(!focusMode)} title="Focus">Focus</ToolbarBtn>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        <div style={{ flex: 1, minHeight: '100vh', backgroundColor: '#fdfcfa', overflowY: 'auto' as const }} onClick={() => editor?.chain().focus().run()}>
          <div style={{ maxWidth: focusMode || isFullscreen ? '740px' : '860px', margin: '0 auto', padding: focusMode || isFullscreen ? '3rem 2rem 6rem' : '2rem 2rem 6rem', transition: 'all 0.2s ease' }}>
            <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Article title..." style={{ width: '100%', fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontFamily: 'Georgia, serif', fontWeight: 700, color: '#0e1a2b', border: 'none', outline: 'none', backgroundColor: 'transparent', lineHeight: 1.2, marginBottom: '1.5rem', boxSizing: 'border-box' as const }} />
            <textarea value={form.excerpt} onChange={e => setForm(f => ({ ...f, excerpt: e.target.value }))} placeholder="Excerpt or standfirst..." rows={2} style={{ width: '100%', fontSize: '18px', color: '#4A5563', fontStyle: 'italic', border: 'none', outline: 'none', backgroundColor: 'transparent', resize: 'none' as const, lineHeight: 1.6, marginBottom: '2rem', borderLeft: '3px solid #c9b28f', paddingLeft: '1rem', boxSizing: 'border-box' as const }} />
            {showHtml ? (
              <textarea value={editor?.getHTML() || ''} onChange={e => editor?.commands.setContent(e.target.value)} style={{ width: '100%', minHeight: 400, fontSize: 13, fontFamily: 'monospace', border: '1px solid #e8e4de', padding: '1rem', outline: 'none', resize: 'vertical' as const, boxSizing: 'border-box' as const }} />
            ) : (
        ...
        {!focusMode && !isFullscreen && (
          <div style={{ width: '320px', flexShrink: 0, backgroundColor: '#fff', borderLeft: '1px solid #e8e4de', overflowY: 'auto' as const, padding: '1.5rem' }}>
            ...
            <div style={{ marginBottom: '1.5rem', paddingBottom: '1.5rem', borderBottom: '1px solid #f0ede8' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '0.4rem' }}>
                {Object.entries(STATUS_CONFIG).map(([key, conf]) => (
                  <button key={key} type="button" onClick={() => setForm(f => ({ ...f, status: key }))} style={{ padding: '4px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase' as const, border: '1px solid', borderColor: form.status === key ? conf.color : '#e8e4de', backgroundColor: form.status === key ? conf.bg : '#fff', color: form.status === key ? conf.color : '#9a9085', cursor: 'pointer', borderRadius: 3 }}>{conf.label}</button>
                ))}
              </div>
              {form.status === 'in_review' && (
                <div style={{ marginTop: '0.75rem' }}>
                  <label style={{ fontSize: '11px', color: '#9a9085', display: 'block', marginBottom: '0.3rem' }}>Review Note (optional)</label>
                  <textarea value={form.review_note} onChange={e => setForm(f => ({ ...f, review_note: e.target.value }))} rows={2} placeholder="Optional note for the writer..." style={{ width: '100%', padding: '0.5rem', border: '1px solid #e8e4de', fontSize: '12px', outline: 'none', resize: 'vertical' as const, boxSizing: 'border-box' as const }} />
                </div>
              )}
            </div>
            {/* Category */}
            ...
            <div style={{ marginBottom: '1.25rem', paddingBottom: '1.25rem', borderBottom: '1px solid #f0ede8' }}>
              ...
              {form.cover_image_url && <img src={form.cover_image_url} alt="cover" style={{ width: '100%', aspectRatio: '16/9', objectFit: 'cover', marginTop: '0.5rem', display: 'block' }} />}
            </div>
            {/* SEO */}
            <div style={{ marginBottom: '1.25rem', paddingBottom: '1.25rem', borderBottom: '1px solid #f0ede8' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '0.3rem', marginBottom: '0.5rem' }}>
                {(form.tags || []).map((t: string) => (
                  <span key={t} style={{ fontSize: '11px', backgroundColor: '#f0ede8', color: '#4A5563', padding: '3px 8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    {t}<button type="button" onClick={() => setForm(f => ({ ...f, tags: f.tags.filter(x => x !== t) }))} style={{ background: 'none', border: 'none', color: '#c9b28f', cursor: 'pointer', fontSize: 14, lineHeight: 1, padding: 0 }}>×</button>
                  </span>
                ))}
              </div>
...
export default function EditV2({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!slug) return null
  return <Suspense><EditV2Inner slug={slug} /></Suspense>
}
