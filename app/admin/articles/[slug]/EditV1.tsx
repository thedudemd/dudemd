...
function EditArticleInner({ slug }: { slug: string }) {
  ...
  const [pillarSearchResults, setPillarSearchResults] = useState<any[]>([])
  const [pendingPillar, setPendingPillar] = useState<any>(null)
  ...
  const [form, setForm] = useState({
    ...
    facebook_teaser_text: '', external_url: '', subcategory_id: '', layout: 'standard', tags: [] as string[], show_hero: true,
    is_pillar_content: false, pillar_topic_id: ''
  })
  ...
  // Fetch cluster articles for pillar view
  async function fetchClusterArticles(id: string) {
    const { data } = await supabase.from('articles').select('id,title,slug,status,categories!articles_category_id_fkey(slug)').eq('pillar_topic_id', id).order('title')
    setClusterArticles(data || [])
  }

  // Search for pillar articles by title
  async function searchPillarArticles(query: string) {
    if (!query.trim()) { setPillarSearchResults([]); return }
    const { data } = await supabase.from('articles').select('id,title,slug,categories!articles_category_id_fkey(slug)').eq('is_pillar_content', true).ilike('title', '%' + query + '%').limit(8)
    setPillarSearchResults(data || [])
  }

  // Confirm and set parent pillar on current article
  async function confirmSetParentPillar(pillar: any) {
    const { error } = await supabase.from('articles').update({ pillar_topic_id: pillar.id }).eq('id', articleId)
    if (error) { alert('Error: ' + error.message); return }
    setForm(f => ({ ...f, pillar_topic_id: pillar.id }))
    setPendingPillar(null)
    setPillarSearch('')
    setPillarSearchResults([])
    // Refresh pillar articles list
    const { data: pillars } = await supabase.from('articles').select('id,title,slug,categories!articles_category_id_fkey(slug)').eq('is_pillar_content', true).order('title')
  ...
  function calcScores(title: string, content: string, words: number) {
    ...
    let seo = 0
    if (title.length >= 30 && title.length <= 65) seo += 15; else if (title.length > 0) seo += 7
    if (metaTitleLen >= 30 && metaTitleLen <= 65) seo += 15; else if (metaTitleLen > 0) seo += 7
    if (metaDescLen >= 120 && metaDescLen <= 160) seo += 15; else if (metaDescLen > 0) seo += 7
    ...
    aeo += Math.min(20, questionWords.filter(w => tl.includes(w)).length * 5)
    if (content.includes('?')) aeo += 15
    if (words >= 800) aeo += 20; else if (words >= 400) aeo += 10
    if (form.excerpt.length >= 100) aeo += 15; else if (form.excerpt.length > 0) aeo += 7
    if (hasH2 && hasH3) aeo += 15; else if (hasH2) aeo += 8
    if ((content.match(/<li/g) || []).length >= 3) aeo += 15
    setAeoScore(Math.min(100, aeo))
    const avg = sentences.length > 0 ? words / sentences.length : 0
    let read = 100
    if (avg > 30) read -= 30; else if (avg > 25) read -= 20; else if (avg > 20) read -= 10
    if (words < 300) read -= 25; else if (words < 150) read -= 40
    const longPct = sentences.length > 0 ? sentences.filter(s => s.split(' ').length > 30).length / sentences.length : 0
  ...
  async function handleAddCategory() {
    ...
    if (error) { alert('Error: ' + error.message) }
      ...
      if (newCatParent) { setSubcategories(s => [...s, data]) }
      else { setCategories(cats => [...cats, data]) }
      setNewCatName('')
      setNewCatParent('')
      setShowAddCat(false)
    }
  ...
  useEffect(() => {
    async function init() {
      ...
      // Fetch all pillar articles for parent pillar selection
      const { data: pillars } = await supabase.from('articles').select('id,title,slug,categories!articles_category_id_fkey(slug)').eq('is_pillar_content', true).order('title')
      setPillarArticles(pillars || [])
      setForm({
        ...
        is_pillar_content: article.is_pillar_content || false, pillar_topic_id: article.pillar_topic_id || ''
      })
  ...
  function handleTitleChange(e: any) {
    ...
    setForm(f => ({ ...f, title, meta_title: title, social_title: title }))
  }
  ...
  async function handleSave(status: string) {
    ...
    const { error } = await supabase.from('articles').update({
      ...cleanForm, content: editor?.getHTML() || '', read_time: getReadTime(), status,
      published: status === 'published', published_at: status === 'published' ? new Date().toISOString() : null
    }).eq('id', articleId)
        ...
        <div className="container-content" style={{ paddingTop: '2rem', paddingBottom: '4rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: fullscreen ? '1fr' : '1fr 300px', gap: '2rem', alignItems: 'start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: fullscreen ? '0' : '1.5rem' }}>
              ...
              <div style={{ display: fullscreen ? 'none' : 'block', backgroundColor: '#fff', border: '1px solid #ede8df', padding: '1.5rem' }}>
                ...
                {suggestions.existing.length > 0 && (
                  <div style={{ marginTop: '1.25rem' }}>
                    ...
                    {suggestions.existing.map((article, idx) => <div key={idx} style={{ fontSize: '13px', color: '#0e1a2b', marginBottom: '0.4rem', paddingLeft: '0.75rem', borderLeft: '2px solid #c9b28f' }}>• {article}</div>)}
                  </div>
                )}
                {suggestions.topics.length > 0 && (
                  <div style={{ marginTop: '1.25rem' }}>
                    ...
                    {suggestions.topics.map((topic, idx) => <div key={idx} style={{ fontSize: '13px', color: '#0e1a2b', marginBottom: '0.4rem', paddingLeft: '0.75rem', borderLeft: '2px solid #c9b28f' }}>• {topic}</div>)}
                  </div>
                )}
              </div>
              <div style={{ display: fullscreen ? 'none' : 'block', backgroundColor: '#fff', border: '1px solid #ede8df', padding: '1.5rem' }}>
              ...
              <div style={{ display: fullscreen ? 'none' : 'block', backgroundColor: '#fff', border: '1px solid #ede8df', padding: '1.5rem' }}>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <textarea name="social_description" value={form.social_description} onChange={handleChange} rows={2} style={{ ...inp, resize: 'vertical' as const }} />
                </div>
                <div>
            ...
            <div style={{ display: fullscreen ? 'none' : 'flex', flexDirection: 'column', gap: '1.5rem', position: 'sticky', top: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '0.4rem', marginBottom: '0.5rem' }}>
                    {(form.tags||[]).map((t:string) => (
                      <span key={t} style={{ fontSize: '11px', backgroundColor: '#0e1a2b', color: '#f7f4ee', padding: '0.25rem 0.6rem', borderRadius: 20, display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                        {t}<button type="button" onClick={() => setForm(f => ({...f, tags: f.tags.filter((x:string)=>x!==t)}))} style={{ background: 'none', border: 'none', color: '#c9b28f', cursor: 'pointer', fontSize: 14, lineHeight: 1, padding: 0 }}>×</button>
                      </span>
                    ))}
                  </div>
              ...
              {/* ARTICLE STRUCTURE — Pillar/Cluster */}
              <div style={{ border: '1px solid #ede8df', backgroundColor: '#fafaf8', marginBottom: '1rem' }}>
                <div style={{ padding: '0.75rem 1rem', backgroundColor: '#0e1a2b' }}>
                  <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: '#c9b28f', margin: 0 }}>Article Structure</p>
                </div>
                <div style={{ padding: '1rem' }}>
                  ...
                  <div style={{ marginBottom: '1rem' }}>
                    <label style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', cursor: 'pointer' }}>
                      <input type="checkbox" checked={form.is_pillar_content} onChange={e => { const checked = e.target.checked; setForm(f => ({ ...f, is_pillar_content: checked, pillar_topic_id: checked ? '' : f.pillar_topic_id })); if (checked && articleId) fetchClusterArticles(articleId) }} style={{ marginTop: '2px', accentColor: '#0e1a2b', width: 16, height: 16, flexShrink: 0 }} />
                      <div>
                  ...
                  {/* Connected clusters — pillar view */}
                  {form.is_pillar_content && (
                    <div style={{ marginBottom: '1rem' }}>
                      <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#4A5563', marginBottom: '0.5rem' }}>Connected Cluster Articles</p>
                        ...
                        : clusterArticles.map((a: any) => (
                          <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0.75rem', backgroundColor: '#fff', border: '1px solid #ede8df', marginBottom: '0.35rem' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
                              <span style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' as const, color: a.status === 'published' ? '#2d7a3a' : '#9a9085', backgroundColor: a.status === 'published' ? '#e8f5ea' : '#f0ede8', padding: '2px 6px' }}>{a.status === 'published' ? 'Published' : 'Draft'}</span>
                              <a href={'/admin/articles/' + a.slug + '/edit'} style={{ fontSize: '11px', color: '#c9b28f', textDecoration: 'none', fontWeight: 700 }}>Edit</a>
                            </div>
                          </div>
                        ))
                      }
                      <button type="button" onClick={() => fetchClusterArticles(articleId)} style={{ fontSize: '11px', color: '#9a9085', background: 'none', border: 'none', cursor: 'pointer', padding: '0.25rem 0', textDecoration: 'underline' }}>Refresh list</button>
                      <p style={{ fontSize: '11px', color: '#9a9085', fontStyle: 'italic' as const, marginTop: '0.5rem' }}>Search below is for discovery only. To link a cluster, open that article and set its Parent Pillar to this article.</p>

                      {/* Read-only discovery search */}
                      <div style={{ marginTop: '0.75rem' }}>
                        <input type="text" placeholder="Discover potential cluster articles..." value={pillarSearch} onChange={e => { setPillarSearch(e.target.value); searchPillarArticles(e.target.value) }} style={{ width: '100%', padding: '0.6rem 0.75rem', border: '1px solid #ede8df', fontSize: '13px', outline: 'none', boxSizing: 'border-box' as const, backgroundColor: '#fff' }} />
                        {pillarSearchResults.length > 0 && (
                          <div style={{ border: '1px solid #ede8df', borderTop: 'none', backgroundColor: '#fff' }}>
                            {pillarSearchResults.map((a: any) => (
                              <div key={a.id} style={{ padding: '0.6rem 0.75rem', borderBottom: '1px solid #f0ede8' }}>
                                <span style={{ fontSize: '13px', color: '#0e1a2b' }}>{a.title}</span>
                                <a href={'/admin/articles/' + a.slug + '/edit'} style={{ float: 'right', fontSize: '11px', color: '#c9b28f', textDecoration: 'none', fontWeight: 700 }}>Open article →</a>
                              </div>
                            ))}
                          </div>
                        )}
                  ...
                  {/* Parent Pillar selector — cluster view */}
                  {!form.is_pillar_content && (
                    <div style={{ marginBottom: '1rem' }}>
                      <label style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#4A5563', display: 'block', marginBottom: '0.4rem' }}>Parent Pillar</label>
                      {form.pillar_topic_id
                        ? <div style={{ padding: '0.6rem 0.75rem', backgroundColor: '#e8f5ea', border: '1px solid #2d7a3a', marginBottom: '0.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: '13px', color: '#0e1a2b' }}>{pillarArticles.find((p: any) => p.id === form.pillar_topic_id)?.title || 'Parent pillar set'}</span>
                            <button type="button" onClick={() => { setPendingPillar(null); setPillarSearch(''); setPillarSearchResults([]) }} style={{ fontSize: '11px', color: '#a32d2d', background: 'none', border: 'none', cursor: 'pointer' }}>Change</button>
                          </div>
                        : <p style={{ fontSize: '12px', color: '#9a9085', fontStyle: 'italic' as const, marginBottom: '0.5rem' }}>No parent pillar set.</p>
                      }

                      {/* Search to find and set parent pillar */}
                      <input type="text" placeholder="Search for parent pillar..." value={pillarSearch} onChange={e => { setPillarSearch(e.target.value); searchPillarArticles(e.target.value) }} style={{ width: '100%', padding: '0.6rem 0.75rem', border: '1px solid #ede8df', fontSize: '13px', outline: 'none', boxSizing: 'border-box' as const, backgroundColor: '#fff' }} />
                      {pillarSearchResults.length > 0 && !pendingPillar && (
                        <div style={{ border: '1px solid #ede8df', borderTop: 'none', backgroundColor: '#fff' }}>
                          {pillarSearchResults.map((a: any) => (
                            <div key={a.id} onClick={() => setPendingPillar(a)} style={{ padding: '0.6rem 0.75rem', borderBottom: '1px solid #f0ede8', cursor: 'pointer', fontSize: '13px', color: '#0e1a2b' }}>{a.title}</div>
                          ))}
                        </div>
                      )}
                      {pendingPillar && (
                        <div style={{ padding: '0.75rem', backgroundColor: '#fef3e2', border: '1px solid #d4820a', marginTop: '0.5rem' }}>
                          <p style={{ fontSize: '13px', color: '#0e1a2b', marginBottom: '0.5rem' }}>Set <strong>{pendingPillar.title}</strong> as the parent pillar for this article?</p>
                          <div style={{ display: 'flex', gap: '0.5rem' }}>
                            <button type="button" onClick={() => confirmSetParentPillar(pendingPillar)} style={{ padding: '0.4rem 0.9rem', backgroundColor: '#0e1a2b', color: '#f7f4ee', border: 'none', fontWeight: 700, fontSize: '12px', cursor: 'pointer' }}>Confirm</button>
                            <button type="button" onClick={() => setPendingPillar(null)} style={{ padding: '0.4rem 0.9rem', backgroundColor: '#fff', color: '#0e1a2b', border: '1px solid #ede8df', fontWeight: 700, fontSize: '12px', cursor: 'pointer' }}>Cancel</button>
                          </div>
                        </div>
                      )}
                    </div>
                ...
                <button onClick={() => handleSave('published')} disabled={saving} style={{ width: '100%', padding: '0.875rem', backgroundColor: '#0e1a2b', color: '#f7f4ee', fontWeight: 700, fontSize: '13px', letterSpacing: '0.08em', textTransform: 'uppercase' as const, border: 'none', cursor: 'pointer' }}>{saving ? 'Saving...' : 'Update Article'}</button>
              </div>
            </div>
          </div>
        </div>
...
export default function EditV1({ params }: { params: Promise<{ slug: string }> }) {
