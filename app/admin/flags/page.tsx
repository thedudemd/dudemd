...
export default function FlagsPage() {
  ...
  async function toggle(key: string, currentValue: boolean) {
    ...
    setFlags(flags.map(f => f.key === key ? { ...f, enabled: newVal } : f))
    await supabase.from('feature_flags').update({ enabled: newVal }).eq('key', key)
  }
      ...
      <div style={{ padding: '2rem 2.5rem', maxWidth: '1100px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          ...
          <div style={{ backgroundColor: '#fff', border: '1px solid #e8e4de', padding: '0.75rem 1.25rem', textAlign: 'center' }}>
            <p style={{ fontSize: '1.5rem', fontWeight: 800, color: '#0e1a2b', fontFamily: 'Georgia, serif', margin: 0 }}>{enabledCount}<span style={{ fontSize: '12px', fontWeight: 400, color: '#9a9085', fontFamily: 'system-ui' }}>/{flags.length}</span></p>
            ...
            <div key={flag.key} style={{ backgroundColor: '#fff', border: `1px solid ${flag.enabled ? '#d4e8d6' : '#e8e4de'}`, padding: '1rem 1.25rem', display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
              ...
              <button
                ...
                style={{
                  ...
                  backgroundColor: flag.enabled ? '#c9b28f' : '#ddd8d0',
                  position: 'relative', flexShrink: 0
                }}
              >
                <span style={{
                  position: 'absolute', top: '3px', left: flag.enabled ? '21px' : '3px',
